...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Basketball Image Analysis
% Object of interest: The basketball
%
% Processes (same as first two images):
%   1) Preprocessing (read, resize to 512x512)
%   2) Color Space Conversions (RGB->HSV, YCbCr)
%   3) Color Segmentation (HSV thresholding for orange ball + morphological cleanup)
%   4) Edge Detection (Sobel, Prewitt) + Enhancement (dilation)
%   5) K-means Clustering
%   6) Object Detection (Connected Components) + Thicker bounding boxes
%   7) Color Overlay (turn the ball red)
%   8) Background Blurring
%   9) Single-Figure Visualization (12 subplots)
%
% NOTE:
%   - Tweak the HSV thresholds for the basketball's color.
%   - If the ball is not segmented well, adjust morphological operations.
%   - Ensure "basketball.jpg" is in your MATLAB folder or path.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

close all; clear; clc;

%% 1) READ & RESIZE IMAGE
originalImg = imread('basketball.jpg');
basketImg = imresize(originalImg, [512 512]);
grayBasket = rgb2gray(basketImg);

%% 2) COLOR SPACE CONVERSIONS
hsvBasket   = rgb2hsv(basketImg);
ycbcrBasket = rgb2ycbcr(basketImg);

%% 3) COLOR SEGMENTATION (HSV Example for Orange Ball)
% Let's define approximate thresholds for an orange color in HSV.
% These ranges are guesses; adjust for your specific image.
H = hsvBasket(:,:,1);  % Hue in [0,1]
S = hsvBasket(:,:,2);  % Saturation in [0,1]
V = hsvBasket(:,:,3);  % Value in [0,1]

% Example ranges for an orange basketball:
%   Hue ~ 0.02 to 0.10
%   Saturation ~ 0.5 to 1
%   Value ~ 0.2 to 1
BW_ball = (H >= 0.02 & H <= 0.10) & ...
          (S >= 0.5) & ...
          (V >= 0.2);

% Morphological cleanup
BW_ball = imclose(BW_ball, strel('disk', 5));  % unify ball region
BW_ball = imfill(BW_ball, 'holes');            % fill holes inside the ball
BW_ball = bwareaopen(BW_ball, 100);            % remove small noise

% Create a color-segmentation overlay (only show the ball region)
colorSegmentedOverlayBall = basketImg;
colorSegmentedOverlayBall(repmat(~BW_ball, [1 1 3])) = 0;

%% 4) EDGE DETECTION & ENHANCEMENT
BW_sobel_basket   = edge(grayBasket, 'sobel');
BW_prewitt_basket = edge(grayBasket, 'prewitt');

% Dilation to emphasize edges
se = strel('disk', 1);
BW_sobel_basket_dilated = imdilate(BW_sobel_basket, se);

%% 5) K-MEANS CLUSTERING
numClusters = 3;
[m, n, c] = size(basketImg);
pixelData = double(reshape(basketImg, [], c));
...
clusteredImgBall = zeros(m, n, 3, 'uint8');
colors = uint8(255 * lines(numClusters));
for i = 1:numClusters
    mask = (pixelLabels == i);
    for ch = 1:3
        temp = clusteredImgBall(:,:,ch);
        temp(mask) = colors(i,ch);
        clusteredImgBall(:,:,ch) = temp;
    end
end

%% 6) OBJECT DETECTION (Connected Components)
% We'll detect the ball using BW_ball
CC = bwconncomp(BW_ball);
stats = regionprops(CC, 'BoundingBox', 'Centroid');
...
objectDetectionOverlayBall = basketImg;
if ~isempty(rectangles)
    objectDetectionOverlayBall = insertShape(basketImg, 'Rectangle', rectangles, ...
        'Color', 'green', 'LineWidth', 3);
end
...
if ~isempty(centroids)
    objectDetectionOverlayBall = insertMarker(objectDetectionOverlayBall, centroids, ...
        'x', 'Color', 'red', 'Size', 10);
end

%% 7) TURN THE BALL RED
ballRedOverlay = basketImg;
Rchan = ballRedOverlay(:,:,1);
Gchan = ballRedOverlay(:,:,2);
Bchan = ballRedOverlay(:,:,3);

Rchan(BW_ball) = 255;
Gchan(BW_ball) = 0;
Bchan(BW_ball) = 0;

ballRedOverlay(:,:,1) = Rchan;
ballRedOverlay(:,:,2) = Gchan;
ballRedOverlay(:,:,3) = Bchan;

%% 8) BACKGROUND BLURRING
% Blur the entire image, keep the ball region unblurred
blurredBasket = imgaussfilt(basketImg, 10);
basketBackgroundBlurred = basketImg;
basketBackgroundBlurred(~repmat(BW_ball, [1 1 3])) = ...
    blurredBasket(~repmat(BW_ball, [1 1 3]));

%% 9) SINGLE-FIGURE VISUALIZATION (12 Subplots)
figure('Name','Basketball Image Analysis','Position',[70 70 1000 700]);
...
% (4) Sobel Edges
subplot(3,4,4);
imshow(BW_sobel_basket);
title('Sobel Edges');

% (5) Prewitt Edges
subplot(3,4,5);
imshow(BW_prewitt_basket);
title('Prewitt Edges');

% (6) Enhanced Edges (Dilated)
subplot(3,4,6);
imshow(BW_sobel_basket_dilated);
title('Enhanced Edges');

% (7) Color Segmentation Mask
subplot(3,4,7);
imshow(BW_ball);
title('Color Segmentation Mask');

% (8) Segmented Overlay (Ball Only)
subplot(3,4,8);
imshow(colorSegmentedOverlayBall);
title('Segmented Overlay');

% (9) K-means Segmentation
subplot(3,4,9);
imshow(clusteredImgBall);
title('K-means Segmentation');

% (10) Object Detection Overlay
subplot(3,4,10);
imshow(objectDetectionOverlayBall);
title('Object Detection');

% (11) Ball Turned Red
subplot(3,4,11);
imshow(ballRedOverlay);
title('Ball Turned Red');

% (12) Background Blurred
subplot(3,4,12);
imshow(basketBackgroundBlurred);
title('Background Blurred');
